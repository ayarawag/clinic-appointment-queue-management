...
Patient::Patient(std::string n, std::string p, std::string e, std::string pass) {
    id=0; name=n; phone=p; email=e; passwordHash=hashPassword(pass);
}

bool Patient::emailExists(std::string email,const std::string& dbfile){
    DBConnection db(dbfile);
    bool found=false;
    db.query("SELECT id FROM patients WHERE email='"+email+"' LIMIT 1;",
        [](void* u,int c,char** v,char**) { if(v[0]) *(bool*)u=true; return 0; }, &found);
    return found;
}
...
Patient Patient::loadById(int pid,const std::string& db){
    Patient p;
    DBConnection dbx(db);
    dbx.query("SELECT id,name,phone,email,password_hash FROM patients WHERE id="+std::to_string(pid),
...
bool Patient::registerPatient(const std::string& db){
    ...
    q<<"INSERT INTO patients(name,phone,email,password_hash,createdAt) VALUES('"
     <<name<<"','"<<phone<<"','"<<email<<"','"<<passwordHash<<"',"<<nowEpochSeconds()<<");";
...
bool Patient::update(const std::string& db){
    ...
    DBConnection x(db);
    std::ostringstream q;
    q<<"UPDATE patients SET name='"<<name<<"', phone='"<<phone<<"', email='"<<email
     <<"' WHERE id="<<id;
    return x.execute(q.str());
}
